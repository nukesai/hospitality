...
describe("runUpgrade", () => {
  ...
  it("rewrites stale pristine files, preserves edits, bumps the manifest", async () => {
    ...
    await writeFile(path.join(cwd, "server/routers/health.ts"), stamp("// old template\n"));
    ...
    expect(byFile["server/routers/health.ts"]).toBe("updated");
    expect(byFile["i18n/request.ts"]).toBe("conflicted");
    expect(byFile["instrumentation.ts"]).toBe("skipped");

    expect(await readFile(path.join(cwd, "server/routers/health.ts"), "utf8")).toContain(
      "healthRouter",
    );
  ...
  it("dry-run plans without writing", async () => {
    ...
    const entry = report.plan.find((item) => item.file === "server/routers/health.ts");
    expect(entry?.action).toBe("updated");
    expect(await readFile(path.join(cwd, "server/routers/health.ts"), "utf8")).toBe(
      stamp("// old template\n"),
    );
